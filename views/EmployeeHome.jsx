...
export default function EmployeeHome() {
  ...
  return (
    <div className="screen">
              ...
              return (
                <li key={swap.id} className="swap-row">
                  ...
                  <span className="swap-desc">
                    {isOutgoing ? `You swapped a task to ${to?.name}` : `${from?.name} swapped a task to you`}
                  </span>
                  {swap.reason && <span className="swap-note">{swap.reason}</span>}
                </li>
              );
            })}
          </ul>
      ...
      {swapTarget     && <SwapModal           task={swapTarget}     onClose={() => setSwapTarget(null)} />}
      {completeTarget && <TaskCompletionModal task={completeTarget} onClose={() => setCompleteTarget(null)} />}
    </div>
  );
}
